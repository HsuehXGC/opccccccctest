...
export const MEETING_KIND: Record<MeetingKind, { label: string; hint: string }> = {
  ...
  review: { label: '发布评审', hint: '自驾发布测试版后，团队对照 PRD 评估交付、给出下一轮建议（由 Navo7 自动发起）' },
}
...
const roundDirective = (round: number) =>
  [
    ...
    '① 针对**具体、有依据**的点，说你同意/不同意谁的哪一点（点名到角色）——只对基于事实的观点表态，不要为表态而表态；',
    '② 据此**更新或收敛**你的立场，或抛出需要拍板的分歧；',
    '③ 只说增量，**不要重复上一轮**。控制在 150–320 字，直接给内容。',
    '⚠️ 不要把别人上一轮**临时编造的编号/代号/决议**（N/D/K/M0-x 之类，知识库里没有的）当成既定前提继续引用或加码——发现有人在凭空造框架，直接指出「这条无依据/需调研」。',
  ].join('\n')
...
// 防跑偏纪律：注入到每个发言人与产品经理整理的提示词，避免会议自我编造、越聊越假。
const GROUNDING_RULES = [
  '',
  '## 纪律（务必遵守）',
  '- **只基于上面「背景知识库」里真实存在的信息发言。** 需要外部事实（真实竞品名、公司、价格、市场规模、行业数据）而知识库里没有时——你手上没有这些，**不要凭空编造**；直接说「此处需联网调研」或标注「（假设，待验证）」。',
  '- **不要发明编号/代号体系（如 N1–N9、D-23、K-11、M0-x）并把它当成既定事实。** 除非知识库里真有，否则不要引用「上一轮的 D-x 决议 / K-x 约束」——那些多半是会上临时编的，会越滚越假。',
  '- 少做「划边界 / 表态 / 谁归谁」这类流程空谈，多给**具体、可验证**的内容；明确区分「事实」与「我的判断（推断）」。',
].join('\n')
...
export function docAuthorPrompt(bot: Bot, meeting: Meeting, product: Product | null, knowledge: string, item: DocManifestItem): string {
  ...
  return [
    ...
    '- 首行用 `# 标题` 开头。',
    '- **若缺少关键信息以致无法写出可交付的文档**（而非可以合理假设的细节），不要编造：请**只输出**一行 `===NEED_INPUT===`，紧接一段说明「还缺什么、需要用户补充哪些信息」，不要输出正文。',
  ].join('\n')
}
...
export function pmTail(kind: MeetingKind): string {
  ...
  return [
    ...
    '详细展开：① 背景与目标 ② 各角色关键意见汇总 ③ 达成的决策 ④ 遗留问题与风险 ⑤ 后续行动项（谁、做什么、何时）。',
    '',
    '⚠️ 整理纪律：**只把真正达成的、有依据的共识写成「决策」**；讨论中临时编造、知识库里查无实据的编号/代号/数字（N/D/K/M0-x、市场规模、竞品名等）**不要写成既定事实**——要么删掉，要么明确标注「（假设，待验证 / 需联网调研）」。区分「事实」与「判断」。宁可短而实，不要长而虚。',
  ].join('\n')
}
