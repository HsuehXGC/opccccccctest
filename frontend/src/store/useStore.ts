...
/** 顶层导航视图 */
export type View = 'dashboard' | 'requirements' | 'wiki' | 'kanban' | 'workforce' | 'meetings' | 'releases' | 'secretary' | 'account'
...
export const useStore = create<State>()(
  ...
  // ── 会议 ──────────────────────────────────────────────
  createMeeting: ({ title, agenda, kind, productId, participantBotIds, references, fullDocSlugs, parallel, rounds }) => {
    ...
    set((s) => {
      ...
      const meeting: Meeting = {
        ...
        rounds: Math.min(3, Math.max(1, rounds ?? 1)),
        messages: [],
        output: '',
        createdAt: Date.now(),
      }
